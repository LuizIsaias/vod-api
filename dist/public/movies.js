...
router.get("/v1/movies", async (req, res) => {
    ...
    const repo = data_source_1.AppDataSource.getRepository(VodItem_1.VodItem);
    const query = repo.createQueryBuilder("movie")
        ...
        .take(take);
    if (q)
        query.andWhere("LOWER(movie.titleNormalized) LIKE :q", { q: `%${String(q).toLowerCase()}%` });
    if (category_id)
        query.andWhere("category.id = :category_id", { category_id });
    if (year)
        query.andWhere("substr(tmdb.releaseDate, 1, 4) = :year", { year });
    if (min_rating)
        query.andWhere("tmdb.voteAverage >= :rating", { rating: Number(min_rating) });
